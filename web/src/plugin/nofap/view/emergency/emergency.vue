  ...
  <div>
      ...
      >
        ...
        <el-form-item label="分类">
          <el-select v-model="searchInfo.category" placeholder="选择分类" clearable>
            ...
            <el-option label="语录" value="quote" />
          </el-select>
        </el-form-item>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="预览" prop="content" min-width="200">
          <template #default="scope">
    ...
    >
      <template #header>
        <div class="flex justify-between items-center">
          <span class="text-lg">{{ type === 'create' ? '添加紧急资源' : '编辑紧急资源' }}</span>
          <div>
      ...
      >
        ...
        <el-form-item label="预览图:" prop="imageUrl">
          <SelectFile v-model="formData.imageUrl" />
        </el-form-item>
...
const getTableData = async () => {
  const res = await getEmergencyResourceList({
    page: page.value,
...
// 删除行
const deleteRow = (row) => {
  ElMessageBox.confirm('确定要删除这条资源吗?', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    const res = await deleteEmergencyResource(row.ID)
    if (res.code === 0) {
...
const enterDialog = async () => {
  elFormRef.value?.validate(async (valid) => {
    ...
    if (type.value === 'create') {
      res = await createEmergencyResource(formData.value)
    } else {
      res = await updateEmergencyResource(formData.value)
    }
    if (res.code === 0) {
