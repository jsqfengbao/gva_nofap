  ...
  <div>
    <div class="gva-search-box">
      ...
      >
        ...
        <el-form-item label="状态">
          <el-select v-model="searchInfo.status" placeholder="帖子状态" clearable>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="标题" prop="title" min-width="180" />
        ...
        <el-table-column align="left" label="回复数" prop="replyCount" width="80" />
...
// 重置
const onReset = () => {
  searchInfo.value = { title: '', status: null }
  getTableData()
}
...
const changeStatus = (row, newStatus) => {
  ...
  ElMessageBox.confirm(`确定要将此帖子设为${statusText}吗?`, '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    const res = await updateCommunityPostStatus(row.ID, { status: newStatus })
    if (res.code === 0) {
...
// 删除行
const deleteRow = (row) => {
  ElMessageBox.confirm('确定要删除该帖子吗?此操作不可恢复!', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
